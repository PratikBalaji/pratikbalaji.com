...
import { MessageSquare, Send, Loader2, X, Bot, User, Sparkles } from 'lucide-react';
...
export default function ChatAssistant() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: 'assistant', content: "Hi! I'm Pratik's AI Resume Assistant. Ask me about his skills, experience, or projects!" },
  ]);
  ...
  const send = async () => {
    ...
    try {
      const resp = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({ messages: allMessages.filter(m => m.role !== 'assistant' || m.content !== messages[0]?.content).map(m => ({ role: m.role, content: m.content })) }),
      });
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-accent text-accent-foreground flex items-center justify-center shadow-lg hover:shadow-[var(--electric-glow)] transition-shadow"
